...
This is a full, extensive tutorial adventure walking the player through basic IF commands. Simply include this extension and the player can start the tutorial at any time with the TUTORIAL command.

If you want to control when the player can access the tutorial, use

	To decide if the player can tutorial:
		if some condition, yes;
		otherwise no.
...
Example: * Starting With the Tutorial - The tutorial runs immediately.
...
A basic example of using the tutorial when play begins.